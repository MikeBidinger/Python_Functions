...
def list_to_dict(list:list, list_widths:list, list_headers:list=[], key_pos:int=0, key_offset:int=0):
    dict = {}
    headerOffset = 0
    # Get header
    if list_headers == []:
        ...
        if list[i] != ['']:
            ...
            for widthHeader in list_widths:
                if valKey not in dict:
                    dict[valKey] = {}
                dict[valKey][list_headers[cnt]] = list[i][cntPos:widthHeader+cntPos].strip()
                cntPos += widthHeader
                cnt += 1
    return dict
...
def dict_to_csv(dict:dict, id_header:str, seperator:str=';', replace_seperator:bool=False):
    list = []
    header_list = []
